...
import { StrategicHint, AiResponse, DebugInfo, TargetCandidate, DifficultyLevel } from "../types";
...
export const getStrategicHint = async (
  ...
  if (difficulty === 'easy') {
    difficultyContext = "Mode: EASY. The board is sparse. Suggest simple, confidence-building shots.";
  } else if (difficulty === 'medium') {
    difficultyContext = "Mode: MEDIUM. Balance scoring with board management.";
  } else {
    difficultyContext = "Mode: HARD. The board is dense and dangerous. Prioritize survival and clearing low-hanging threats over pure score.";
  }
  ...
  debug.promptContext = fullPromptContext;
  ...
  const prompt = `
    ...
    - Danger Level: ${dangerRow >= (difficulty === 'hard' ? 8 : 6) ? "CRITICAL (Bubbles near bottom!)" : "Stable"}
    ...
    **IMPORTANT**: The 'message' and 'rationale' MUST be in ${language === 'ar' ? 'ARABIC' : 'ENGLISH'}.
    Keep the 'rationale' VERY CONCISE (max 20 words).
    
    JSON structure:
    {
      "message": "Short operational directive (e.g. 'Target the Red Cluster')",
      "rationale": "Brief strategic reason.",
  ...
  try {
    ...
    try {
        ...
        return {
            hint: getBestLocalTarget("System returned invalid coordinates"),
        ...
        return {
            hint: getBestLocalTarget("System response parse error"),
    ...
    debug.latency = Math.round(endTime - startTime);
    return {
        hint: getBestLocalTarget("Service Unreachable"),
